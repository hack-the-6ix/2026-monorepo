...
interface ApplicationFailureToastProps {
  onDismiss: () => void;
}

export default function ApplicationFailureToast({
  onDismiss,
}: ApplicationFailureToastProps) {
  return (
    <div
      role="alert"
      aria-live="assertive"
    ...
    >
      <span
        className="flex size-6 shrink-0 items-center justify-center rounded-full bg-error-400 text-white"
        aria-hidden
      >
        <AlertCircle className="size-3.5 stroke-[2.5]" />
      </span>
      <p className="min-w-0 flex-1 text-sm font-semibold text-error-600 md:text-[0.9375rem]">
